...
bool GLWindow::create(int width, int height, int bpp, bool fullscreen)
{
	...
	else
	{
		m_windowRect.left = (long)0; // Set Left Value To 0
		m_windowRect.right = (long)width; // Set Right Value To Requested Width
		m_windowRect.top = (long)-height;  // Set Top Value To 0
		m_windowRect.bottom = (long)0;   // Set Bottom Value To Requested Height
	}
...
void GLWindow::attachExample(OpenGL* openGL)
{
    m_openGL = openGL;
}
...
LRESULT GLWindow::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_SIZE:
    {
        ...
        getAttachedExample()->Resize(width, height); //Call the example's resize method
    }
    break;
    case WM_KEYDOWN:
...
bool GLWindow::KeyPressed(char letter)
{
	if (curKeyState[letter] == 1 && prevKeyState[letter] == 0)
		return true;
	else
		return false;
}
bool GLWindow::KeyDown(char letter)
{
 	if (curKeyState[letter] == 1 && prevKeyState[letter] == 1)
		return true;
	else
		return false;
}
bool GLWindow::KeyUp(char letter)
{
	if (curKeyState[letter] == 0)
		return true;
	else
		return false;
}
float GLWindow::GetMouseX()
{
	return mousePos.x;
}
float GLWindow::GetMouseY()
{

	if (FlipY == false)
	{
		return ScreenY - mousePos.y;
	}
	else
	{
		return mousePos.y;
